...
999 ERRORSEXITS("FiniteElasticity_CylinderAnalyticCalculate",ERR,ERROR)
  ...
  SUBROUTINE FINITE_ELASTICITY_GAUSS_ELASTICITY_TENSOR(EQUATIONS_SET,LAGRANGE_INTERPOLATED_POINT, &
      & MATERIALS_INTERPOLATED_POINT,ELASTICITY_TENSOR,STRESS_TENSOR,DZDNU,Jznu, &
    ...
    TYPE(FIELD_INTERPOLATED_POINT_TYPE), POINTER :: LAGRANGE_INTERPOLATED_POINT,MATERIALS_INTERPOLATED_POINT
    ...
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
    INTEGER(INTG) :: i,j,dof_idx
    REAL(DP) :: lambda,p,pTilde,kappa
    ...
    CASE(EQUATIONS_SET_MOONEY_RIVLIN_SUBTYPE,EQUATIONS_SET_MOONEY_RIVLIN_ACTIVECONTRACTION_SUBTYPE)
      lambda=LAGRANGE_INTERPOLATED_POINT%VALUES(1,NO_PART_DERIV)
      ...
      !Add stabilisation terms \todo: do this properly
      kappa=2.0_DP
      p=lambda+kappa*(Jznu-1.0_DP)
      pTilde=p+kappa*Jznu
      STRESS_TENSOR(1:3)=STRESS_TENSOR(1:3)+p
      ELASTICITY_TENSOR(1:3,1:3)=ELASTICITY_TENSOR(1:3,1:3)+pTilde
      DO i=1,6
        ELASTICITY_TENSOR(i,i)=ELASTICITY_TENSOR(i,i)-2.0_DP*p*UNITY_DIAGONAL(i)
      ENDDO
...
!     ELASTICITY_TENSOR(2,3)=ELASTICITY_TENSOR(2,3)+P
    ...
    CASE(EQUATIONS_SET_TRANSVERSE_ISOTROPIC_GUCCIONE_SUBTYPE,EQUATIONS_SET_GUCCIONE_ACTIVECONTRACTION_SUBTYPE)
      ...
      !Add volumetric parts.
      STRESS_TENSOR(1:3)=STRESS_TENSOR(1:3)+lambda
      ELASTICITY_TENSOR(1,1)=ELASTICITY_TENSOR(1,1)-lambda
      ELASTICITY_TENSOR(2,2)=ELASTICITY_TENSOR(2,2)-lambda
      ELASTICITY_TENSOR(3,3)=ELASTICITY_TENSOR(3,3)-lambda
      ELASTICITY_TENSOR(4,4)=ELASTICITY_TENSOR(4,4)-lambda
      ELASTICITY_TENSOR(5,5)=ELASTICITY_TENSOR(5,5)-lambda
      ELASTICITY_TENSOR(6,6)=ELASTICITY_TENSOR(6,6)-lambda
      ELASTICITY_TENSOR(2,1)=ELASTICITY_TENSOR(2,1)+lambda
      ELASTICITY_TENSOR(3,1)=ELASTICITY_TENSOR(3,1)+lambda
      ELASTICITY_TENSOR(1,2)=ELASTICITY_TENSOR(1,2)+lambda
      ELASTICITY_TENSOR(3,2)=ELASTICITY_TENSOR(3,2)+lambda
      ELASTICITY_TENSOR(1,3)=ELASTICITY_TENSOR(1,3)+lambda
      ELASTICITY_TENSOR(2,3)=ELASTICITY_TENSOR(2,3)+lambda
    CASE DEFAULT
...
999 ERRORSEXITS("FINITE_ELASTICITY_GAUSS_ELASTICITY_TENSOR",ERR,ERROR)
  ...
  SUBROUTINE FiniteElasticity_FiniteElementJacobianEvaluate(EQUATIONS_SET,ELEMENT_NUMBER,ERR,ERROR,*)
    ...
    INTEGER(INTG) :: ng,nh,ns,nhs,ni,mh,ms,mhs,oh
    ...
    INTEGER(INTG) :: ELEMENT_BASE_DOF_INDEX(3)
    INTEGER(INTG), PARAMETER :: OFF_DIAG_COMP(3)=[0,1,3],OFF_DIAG_DEP_VAR1(3)=[1,1,2],OFF_DIAG_DEP_VAR2(3)=[2,3,3]
    INTEGER(INTG) :: MESH_COMPONENT_NUMBER,NUMBER_OF_ELEMENT_PARAMETERS(3)
    ...
    TYPE(FIELD_INTERPOLATED_POINT_TYPE), POINTER :: GEOMETRIC_INTERPOLATED_POINT,FIBRE_INTERPOLATED_POINT, &
      & MATERIALS_INTERPOLATED_POINT,DEPENDENT_INTERPOLATED_POINT,LAGRANGE_INTERPOLATED_POINT
    ...
    TYPE(FIELD_TYPE), POINTER :: DEPENDENT_FIELD,GEOMETRIC_FIELD,MATERIALS_FIELD,FIBRE_FIELD,LAGRANGE_FIELD
    ...
    IF(ASSOCIATED(EQUATIONS_SET)) THEN
      ...
      IF(ASSOCIATED(EQUATIONS)) THEN
        ...
        IF(JACOBIAN_MATRIX%UPDATE_JACOBIAN) THEN
          ...
          !Get the hydrostatic pressure field. Maybe put lagrange field in equations set type?
          !\todo: change this to something better
          IF(ASSOCIATED(EQUATIONS_SET%REGION%CONSTRAINT_CONDITIONS%CONSTRAINT_CONDITIONS)) THEN
            LAGRANGE_FIELD=>EQUATIONS_SET%REGION%CONSTRAINT_CONDITIONS%CONSTRAINT_CONDITIONS(1)%PTR%LAGRANGE%LAGRANGE_FIELD
          ENDIF
          ...
          LAGRANGE_VARIABLE=>EQUATIONS_SET%REGION%CONSTRAINT_CONDITIONS%CONSTRAINT_CONDITIONS(1)%PTR%CONSTRAINT_EQUATIONS% &
            & CONSTRAINT_MAPPING%LAGRANGE_VARIABLE
          ...
          DEPENDENT_INTERPOLATION_PARAMETERS=>EQUATIONS%INTERPOLATION% &
            & DEPENDENT_INTERP_PARAMETERS(DEPENDENT_VARIABLE_TYPE)%PTR
          LAGRANGE_INTERPOLATION_PARAMETERS=>EQUATIONS_SET%REGION%CONSTRAINT_CONDITIONS%CONSTRAINT_CONDITIONS(1)%PTR% &
            & CONSTRAINT_EQUATIONS%INTERPOLATION%LAGRANGE_INTERP_PARAMETERS(LAGRANGE_VARIABLE_TYPE)%PTR
          MATERIALS_INTERPOLATION_PARAMETERS=>EQUATIONS%INTERPOLATION% &
            & MATERIALS_INTERP_PARAMETERS(FIELD_U_VARIABLE_TYPE)%PTR
          FIBRE_INTERPOLATION_PARAMETERS=>EQUATIONS%INTERPOLATION% &
            & FIBRE_INTERP_PARAMETERS(FIELD_U_VARIABLE_TYPE)%PTR
          ...
          DEPENDENT_INTERPOLATED_POINT=>EQUATIONS%INTERPOLATION%DEPENDENT_INTERP_POINT(DEPENDENT_VARIABLE_TYPE)%PTR
          DEPENDENT_INTERPOLATED_POINT_METRICS=>EQUATIONS%INTERPOLATION% &
            & DEPENDENT_INTERP_POINT_METRICS(DEPENDENT_VARIABLE_TYPE)%PTR
          IF(ASSOCIATED(LAGRANGE_FIELD)) THEN
            LAGRANGE_INTERPOLATED_POINT=>EQUATIONS_SET%REGION%CONSTRAINT_CONDITIONS%CONSTRAINT_CONDITIONS(1)%PTR% &
              & CONSTRAINT_EQUATIONS%INTERPOLATION%LAGRANGE_INTERP_POINT(LAGRANGE_VARIABLE_TYPE)%PTR
          END IF
          IF(ASSOCIATED(FIBRE_FIELD)) THEN
          ...
          DO nh=1,NUMBER_OF_DIMENSIONS
            MESH_COMPONENT_NUMBER=DEPENDENT_VARIABLE%COMPONENTS(nh)%MESH_COMPONENT_NUMBER
          ...
          DO ng=1,DEPENDENT_QUADRATURE_SCHEME%NUMBER_OF_GAUSS
            ...
            IF(ASSOCIATED(LAGRANGE_FIELD)) THEN
              CALL FIELD_INTERPOLATE_GAUSS(NO_PART_DERIV,BASIS_DEFAULT_QUADRATURE_SCHEME,ng, &
                & LAGRANGE_INTERPOLATED_POINT,ERR,ERROR,*999)
            ENDIF

            JGW=DEPENDENT_INTERPOLATED_POINT_METRICS%JACOBIAN*DEPENDENT_QUADRATURE_SCHEME%GAUSS_WEIGHTS(ng)
                ...
                DO mh=1,NUMBER_OF_DIMENSIONS
                  ...
                  DO ni=1,NUMBER_OF_XI
                    SUM1=SUM1+QUADRATURE_SCHEMES(nh)%PTR%GAUSS_BASIS_FNS(ns,PARTIAL_DERIVATIVE_FIRST_DERIVATIVE_MAP(ni),ng)* &
                      & DEPENDENT_INTERPOLATED_POINT_METRICS%DXI_DX(ni,mh)
                  ENDDO !mi
            ...
            CALL FINITE_ELASTICITY_GAUSS_ELASTICITY_TENSOR(EQUATIONS_SET,LAGRANGE_INTERPOLATED_POINT, &
              & MATERIALS_INTERPOLATED_POINT,ELASTICITY_TENSOR,STRESS_TENSOR,DZDNU,Jznu,ELEMENT_NUMBER,ng,ERR,ERROR,*999)
            ...
            DO nh=1,NUMBER_OF_DIMENSIONS
              DO ns=1,NUMBER_OF_ELEMENT_PARAMETERS(nh)
                ...
                DO ms=ns,NUMBER_OF_ELEMENT_PARAMETERS(nh)
                  mhs=mhs+1
                  JACOBIAN_MATRIX%ELEMENT_JACOBIAN%MATRIX(mhs,nhs)=JACOBIAN_MATRIX%ELEMENT_JACOBIAN%MATRIX(mhs,nhs)* &
                    & DEPENDENT_INTERPOLATION_PARAMETERS%SCALE_FACTORS(ms,nh)* &
...
!          ENDIF
    ...
    RETURN
999 ERRORSEXITS("FiniteElasticity_FiniteElementJacobianEvaluate",ERR,ERROR)
    RETURN 1
...
999 ERRORSEXITS("FINITE_ELASTICITY_PUSH_STRESS_TENSOR",ERR,ERROR)
  ...
  SUBROUTINE FiniteElasticity_FiniteElementResidualEvaluate(EQUATIONS_SET,ELEMENT_NUMBER,ERR,ERROR,*)
    ...
    TYPE(FIELD_TYPE), POINTER :: INDEPENDENT_FIELD,LAGRANGE_FIELD
    ...
    TYPE(FIELD_INTERPOLATION_PARAMETERS_TYPE), POINTER :: GEOMETRIC_INTERPOLATION_PARAMETERS, &
      ...
      & DENSITY_INTERPOLATION_PARAMETERS,INDEPENDENT_INTERPOLATION_PARAMETERS,LAGRANGE_INTERPOLATION_PARAMETERS
    TYPE(FIELD_INTERPOLATED_POINT_TYPE), POINTER :: GEOMETRIC_INTERPOLATED_POINT,FIBRE_INTERPOLATED_POINT, &
    ...
    NULLIFY(LAGRANGE_INTERPOLATION_PARAMETERS)
    ...
    NULLIFY(DENSITY_INTERPOLATED_POINT,INDEPENDENT_INTERPOLATED_POINT,LAGRANGE_INTERPOLATED_POINT)
        ...
        CASE(EQUATIONS_SET_MOONEY_RIVLIN_ACTIVECONTRACTION_SUBTYPE,EQUATIONS_SET_MOONEY_RIVLIN_SUBTYPE, &
          ...
          DO gauss_idx=1,DEPENDENT_NUMBER_OF_GAUSS_POINTS
           ...
           !Calculate the Cauchy stress tensor (in Voigt form) at the gauss point.
            CALL FINITE_ELASTICITY_GAUSS_STRESS_TENSOR(EQUATIONS_SET,LAGRANGE_INTERPOLATED_POINT, &
...
999 ERRORSEXITS("FINITE_ELASTICITY_GAUSS_CAUCHY_TENSOR",ERR,ERROR)
  ...
  !>Evaluates the Cauchy stress tensor at a given Gauss point
  SUBROUTINE FINITE_ELASTICITY_GAUSS_STRESS_TENSOR(EQUATIONS_SET,LAGRANGE_INTERPOLATED_POINT, &
    ...
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
    INTEGER(INTG) :: component_idx,dof_idx
    REAL(DP) :: lambda,p,kappa
    REAL(DP) :: I1 !Invariants, if needed
    ...
    SELECT CASE(EQUATIONS_SET%SUBTYPE)
    CASE(EQUATIONS_SET_MOONEY_RIVLIN_ACTIVECONTRACTION_SUBTYPE,EQUATIONS_SET_MOONEY_RIVLIN_SUBTYPE)
      IF(ASSOCIATED(LAGRANGE_INTERPOLATED_POINT)) THEN
        lambda=LAGRANGE_INTERPOLATED_POINT%VALUES(1,NO_PART_DERIV)
      ELSE
        lambda=0.0_DP
      END IF
    ...
    CASE(EQUATIONS_SET_TRANSVERSE_ISOTROPIC_GUCCIONE_SUBTYPE,EQUATIONS_SET_GUCCIONE_ACTIVECONTRACTION_SUBTYPE)
      ...
      !Calculate isochoric Cauchy tensor (the deviatoric part) and add the volumetric part (the hydrostatic pressure).
      ONETHIRD_TRACE=SUM(STRESS_TENSOR(1:3))/3.0_DP
      STRESS_TENSOR(1:3)=STRESS_TENSOR(1:3)-ONETHIRD_TRACE+lambda
    CASE DEFAULT
      LOCAL_ERROR="Equations set subtype "//TRIM(NUMBER_TO_VSTRING(EQUATIONS_SET%SUBTYPE,"*",ERR,ERROR))// &
        & " is not valid for a finite elasticity equation type of an elasticity equation set class."
     CALL FlagError(LOCAL_ERROR,ERR,ERROR,*999)
