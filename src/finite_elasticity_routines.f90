...
999 CALL ERRORS("FINITE_ELASTICITY_CYLINDER_ANALYTIC_CALCULATE",ERR,ERROR)
  ...
  SUBROUTINE FINITE_ELASTICITY_GAUSS_ELASTICITY_TENSOR(EQUATIONS_SET,DEPENDENT_INTERPOLATED_POINT, &
      & HYDROSTATIC_PRESSURE_INTERPOLATED_POINT,MATERIALS_INTERPOLATED_POINT,ELASTICITY_TENSOR,STRESS_TENSOR, &
      & DZDNU,Jznu,ELEMENT_NUMBER,GAUSS_POINT_NUMBER,ERR,ERROR,*)
    !Argument variables
    TYPE(EQUATIONS_SET_TYPE), POINTER, INTENT(IN) :: EQUATIONS_SET !<A pointer to the equations set 
    TYPE(FIELD_INTERPOLATED_POINT_TYPE), POINTER :: DEPENDENT_INTERPOLATED_POINT,HYDROSTATIC_PRESSURE_INTERPOLATED_POINT, &
    ...
    CASE(EQUATIONS_SET_MOONEY_RIVLIN_ACTIVECONTRACTION_SUBTYPE)
      P=HYDROSTATIC_PRESSURE_INTERPOLATED_POINT%VALUES(1,NO_PART_DERIV)
...
999 CALL ERRORS("FINITE_ELASTICITY_GAUSS_ELASTICITY_TENSOR",ERR,ERROR)
  ...
  SUBROUTINE FINITE_ELASTICITY_FINITE_ELEMENT_JACOBIAN_EVALUATE(EQUATIONS_SET,ELEMENT_NUMBER,JACOBIAN_NUMBER,ERR,ERROR,*)
    !Argument variables
  ...
  !          ENDIF
            ...
            DO nhs=2,JACOBIAN_MATRIX%ELEMENT_JACOBIAN%NUMBER_OF_COLUMNS
              DO mhs=1,nhs-1
  ...
  !          !Call surface pressure term here: should only be executed if THIS element has surface pressure on it (direct or incremented)
        ...
        ELSE !Off-diagonal submatrix
          ...
          CASE(EQUATIONS_SET_INCOMPRESSIBILITY_TYPE)
            IF(JACOBIAN_MATRIX%UPDATE_JACOBIAN) THEN
              !See if we can later transpose the other coupling matrix to this one.
              !Maybe put extra logical variable in equations matrix type?
            ENDIF
          CASE DEFAULT
            LOCAL_ERROR="Equations set type "//TRIM(NUMBER_TO_VSTRING(COUPLED_EQUATIONS_SET_TYPE,"*",ERR,ERROR))// &
              & " is not valid for coupling to a finite elasticity equations set type."
            CALL FLAG_ERROR(LOCAL_ERROR,ERR,ERROR,*999)
          END SELECT
        ENDIF
      ELSE
...
999 CALL ERRORS("FINITE_ELASTICITY_PUSH_STRESS_TENSOR",ERR,ERROR)
  ...
  SUBROUTINE FINITE_ELASTICITY_FINITE_ELEMENT_RESIDUAL_EVALUATE(EQUATIONS_SET,ELEMENT_NUMBER,ERR,ERROR,*)
    ...
    TYPE(FIELD_INTERPOLATION_PARAMETERS_TYPE), POINTER :: GEOMETRIC_INTERPOLATION_PARAMETERS, &
      ...
      & DENSITY_INTERPOLATION_PARAMETERS,INDEPENDENT_INTERPOLATION_PARAMETERS,HYDROSTATIC_PRESSURE_INTERPOLATION_PARAMETERS
    TYPE(FIELD_INTERPOLATED_POINT_TYPE), POINTER :: GEOMETRIC_INTERPOLATED_POINT,FIBRE_INTERPOLATED_POINT, &
      ...
      & HYDROSTATIC_PRESSURE_INTERPOLATED_POINT
    ...
    NULLIFY(HYDROSTATIC_PRESSURE_INTERPOLATION_PARAMETERS)
    ...
    NULLIFY(HYDROSTATIC_PRESSURE_INTERPOLATED_POINT)
    ...
    IF(ASSOCIATED(EQUATIONS_SET)) THEN
      ...
      IF(ASSOCIATED(EQUATIONS)) THEN 
        ...
        CASE(EQUATIONS_SET_MOONEY_RIVLIN_ACTIVECONTRACTION_SUBTYPE) ! 4 dependent components, works with 2PK, not Cauchy
          ...
          HYDROSTATIC_PRESSURE_INTERPOLATION_PARAMETERS=>EQUATIONS%INTERPOLATION% &
            & DEPENDENT_INTERP_PARAMETERS(FIELD_V_VARIABLE_TYPE)%PTR
          CALL FIELD_INTERPOLATION_PARAMETERS_ELEMENT_GET(FIELD_VALUES_SET_TYPE,ELEMENT_NUMBER, &
            & HYDROSTATIC_PRESSURE_INTERPOLATION_PARAMETERS,ERR,ERROR,*999)
          HYDROSTATIC_PRESSURE_INTERPOLATED_POINT=>EQUATIONS%INTERPOLATION%DEPENDENT_INTERP_POINT(FIELD_V_VARIABLE_TYPE)%PTR
          DO gauss_idx=1,DEPENDENT_NUMBER_OF_GAUSS_POINTS
            ...
            CALL FIELD_INTERPOLATED_POINT_METRICS_CALCULATE(DEPENDENT_BASIS%NUMBER_OF_XI,DEPENDENT_INTERPOLATED_POINT_METRICS, &
              & ERR,ERROR,*999)
            CALL FIELD_INTERPOLATE_GAUSS(FIRST_PART_DERIV,BASIS_DEFAULT_QUADRATURE_SCHEME,gauss_idx, &
              & HYDROSTATIC_PRESSURE_INTERPOLATED_POINT,ERR,ERROR,*999)
            IF(ASSOCIATED(FIBRE_FIELD)) THEN
            ...
            CALL FINITE_ELASTICITY_GAUSS_STRESS_TENSOR(EQUATIONS_SET,DEPENDENT_INTERPOLATED_POINT, &
              & HYDROSTATIC_PRESSURE_INTERPOLATED_POINT,MATERIALS_INTERPOLATED_POINT,CAUCHY_TENSOR,DZDNU,Jznu, &
...
999 CALL ERRORS("FINITE_ELASTICITY_GAUSS_CAUCHY_TENSOR",ERR,ERROR)
  ...
  SUBROUTINE FINITE_ELASTICITY_GAUSS_STRESS_TENSOR(EQUATIONS_SET,DEPENDENT_INTERPOLATED_POINT, &
    ...
    CASE(EQUATIONS_SET_MOONEY_RIVLIN_ACTIVECONTRACTION_SUBTYPE)
      ...
      DO component_idx=1,NUMBER_OF_DIMENSIONS
        CAUCHY_TENSOR(component_idx,component_idx)=CAUCHY_TENSOR(nh,nh)-ONETHIRD_TRACE+P
      ENDDO
        ...
        CASE(EQUATIONS_SET_SETUP_EQUATIONS_TYPE)
          ...
          CASE(EQUATIONS_SET_SETUP_FINISH_ACTION)
            ...
            CASE(EQUATIONS_SET_FEM_SOLUTION_METHOD)
              ...
              SELECT CASE(EQUATIONS_SET%SUBTYPE)
              CASE(EQUATIONS_SET_MOONEY_RIVLIN_ACTIVECONTRACTION_SUBTYPE)
                !Set the hydrostatic pressure now as a different variable instead of component.
