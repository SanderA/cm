  ...
  SUBROUTINE DATA_POINT_CHECK_EXISTS(DATA_POINTS,USER_NUMBER,DATA_POINT_EXISTS,GLOBAL_NUMBER,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(DATA_POINTS)) THEN
      ...
      IF(ASSOCIATED(TREE_NODE)) THEN
        CALL Tree_NodeValueGet(DATA_POINTS%DATA_POINTS_TREE,TREE_NODE,GLOBAL_NUMBER,ERR,ERROR,*999)
  ...
  SUBROUTINE DATA_POINTS_CREATE_START_GENERIC(DATA_POINTS,NUMBER_OF_DATA_POINTS,NUMBER_OF_DIMENSIONS, &
    ...
    IF(ASSOCIATED(DATA_POINTS)) THEN
      IF(NUMBER_OF_DATA_POINTS>0) THEN
        ...
        CALL Tree_CreateStart(DATA_POINTS%DATA_PROJECTIONS_TREE,ERR,ERROR,*999)
        CALL Tree_InsertTypeSet(DATA_POINTS%DATA_PROJECTIONS_TREE,TREE_NO_DUPLICATES_ALLOWED,ERR,ERROR,*999)
        CALL Tree_CreateFinish(DATA_POINTS%DATA_PROJECTIONS_TREE,ERR,ERROR,*999)
        CALL Tree_CreateStart(DATA_POINTS%DATA_POINTS_TREE,ERR,ERROR,*999)
        CALL Tree_InsertTypeSet(DATA_POINTS%DATA_POINTS_TREE,TREE_NO_DUPLICATES_ALLOWED,ERR,ERROR,*999)
        CALL Tree_CreateFinish(DATA_POINTS%DATA_POINTS_TREE,ERR,ERROR,*999)
        ...
        DO data_point_idx=1,DATA_POINTS%NUMBER_OF_DATA_POINTS
          ...
          ENDDO
          CALL Tree_ItemInsert(DATA_POINTS%DATA_POINTS_TREE,data_point_idx,data_point_idx,INSERT_STATUS,ERR,ERROR,*999)
        ENDDO !data_point_idx
      ELSE
      ...
      ELSE
        IF(GLOBAL_NUMBER>=1.AND.GLOBAL_NUMBER<=DATA_POINTS%NUMBER_OF_DATA_POINTS) THEN
          ...
          IF(DATA_POINT_EXISTS) THEN
            ...
            ENDIF
          ELSE
            CALL Tree_ItemDelete(DATA_POINTS%DATA_POINTS_TREE,DATA_POINTS%DATA_POINTS(GLOBAL_NUMBER)%USER_NUMBER,ERR,ERROR,*999)
            CALL Tree_ItemInsert(DATA_POINTS%DATA_POINTS_TREE,USER_NUMBER,GLOBAL_NUMBER,INSERT_STATUS,ERR,ERROR,*999)
  ...
  SUBROUTINE DataPoints_DataProjectionGlobalNumberGet(DATA_POINTS,USER_NUMBER,GLOBAL_NUMBER,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(DATA_POINTS)) THEN
      IF(DATA_POINTS%DATA_POINTS_FINISHED) THEN
        ...
        IF(ASSOCIATED(TREE_NODE)) THEN
          CALL Tree_NodeValueGet(DATA_POINTS%DATA_PROJECTIONS_TREE,TREE_NODE,GLOBAL_NUMBER,ERR,ERROR,*999)
        ELSE
          LOCAL_ERROR="Tree node is not associates (cannot find the user number "//TRIM(NUMBER_TO_VSTRING(USER_NUMBER,"*",ERR, &
            & ERROR))//"."
          CALL FlagError(LOCAL_ERROR,ERR,ERROR,*999)
