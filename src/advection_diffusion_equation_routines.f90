              ...
              IF(SOLVER%GLOBAL_NUMBER==1) THEN
                ...
                IF(ASSOCIATED(SOLVER_EQUATIONS_ADVECTION_DIFFUSION)) THEN
                  ...
                  IF(ASSOCIATED(SOLVER_MAPPING_ADVECTION_DIFFUSION)) THEN
                    EQUATIONS_SET_ADVECTION_DIFFUSION=>SOLVER_MAPPING_ADVECTION_DIFFUSION% &
                      & EQUATIONS_SET_TO_SOLVER_MAP(1)%EQUATIONS_SET
              ...
              IF(SOLVER%GLOBAL_NUMBER==1) THEN
                ...
                IF(ASSOCIATED(SOLVER_EQUATIONS_DIFFUSION)) THEN
                  ...
                  IF(ASSOCIATED(SOLVER_MAPPING_DIFFUSION)) THEN
                    EQUATIONS_SET_DIFFUSION=>SOLVER_MAPPING_DIFFUSION%EQUATIONS_SET_TO_SOLVER_MAP(1)%EQUATIONS_SET
                ...
                IF(ASSOCIATED(SOLVER_EQUATIONS_ADVECTION_DIFFUSION)) THEN
                  SOLVER_MAPPING_ADVECTION_DIFFUSION=>SOLVER_EQUATIONS_ADVECTION_DIFFUSION%SOLVER_MAPPING
                  IF(ASSOCIATED(SOLVER_MAPPING_ADVECTION_DIFFUSION)) THEN
                    EQUATIONS_SET_ADVECTION_DIFFUSION=>SOLVER_MAPPING_ADVECTION_DIFFUSION%EQUATIONS_SET_TO_SOLVER_MAP(1)% &
                      & EQUATIONS_SET
                    IF(ASSOCIATED(EQUATIONS_SET_ADVECTION_DIFFUSION)) THEN
...
!\todo: Reduce number of variable used
    ...
    INTEGER(INTG), POINTER :: BOUNDARY_NODES(:),conditionTypes(:)
                    ...
                    IF(ASSOCIATED(FIELD_VARIABLE)) THEN
                      ...
                      IF(ASSOCIATED(BOUNDARY_CONDITIONS_VARIABLE)) THEN
                        ...
                        DO node_idx=1,SIZE(BOUNDARY_VALUES)
                          ...
                          END IF
                        ENDDO
                        CALL DistributedVector_DataRestore(BOUNDARY_CONDITIONS_VARIABLE%CONDITION_TYPES,conditionTypes, &
                          & err,error,*999)
                      ELSE
